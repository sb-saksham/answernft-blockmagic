    ...
    <Col md="4" sm="12">
        <Card style={{ width: '18rem' }}>
            ...
            <Card.Body>
                <Card.Text>
                    <Form>
                        {alreadyListed ? <Button variant="info" disabled={withdrawNftError || withdrawNftPrepError} onClick={async () => {
                            ...
                            <Button disabled={listNftPrepError || listNftError} onClick={async () => {
                                await listNftWrite?.();
...
const MyNfts = () => {
    ...
    <Row className="px-5 align-items-center">
        <h2>NFTs owned by {userAddress}</h2>
          {tokensMetadata ? tokensMetadata?.map((nft) => {
